    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (conversation.getAllMsgCount() != 0) {
            ...
            if (lastMessage.direct() == EMMessage.Direct.RECEIVE) {
                try {
                    Glide.with(getContext()).load(lastMessage.getStringAttribute("MESSAGE_ATTR_FORM_PIC")).into(holder.avatar);
                    EaseUserUtils.setUserNick(lastMessage.getStringAttribute("MESSAGE_ATTR_FORM_NICK"), holder.name);
                } catch (HyphenateException e) {
                }
            } else {
                try {
                    EaseUserUtils.setUserNick(lastMessage.getStringAttribute("MESSAGE_ATTR_TO_NICK"), holder.name);
                    Glide.with(getContext()).load(lastMessage.getStringAttribute("MESSAGE_ATTR_TO_PIC")).into(holder.avatar);
                } catch (HyphenateException e) {
                }
            }
        }
